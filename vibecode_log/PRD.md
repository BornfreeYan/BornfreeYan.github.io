...
- **Hero 视觉**：纯 SVG 鎏金线条飞鸟图样（呼应 Born Free，无 3D 依赖）
...
1. **Hero 区域**
   ...
   - 右侧（桌面端）/ 上方（移动端）：SVG 鎏金飞鸟图样（呼应 Born Free）
...
### 6.2 v1.1 及以后
- [x] Hero 右侧视觉：SVG 鎏金飞鸟图样（替代 3D 三角钢琴）
...
- ~~三角钢琴 3D 动画~~：已废弃（多次迭代不理想），改用 SVG 飞鸟图样，避免 Three.js 复杂度与首屏体积。
