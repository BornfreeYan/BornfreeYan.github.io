...
3. 写文字 + 图片
   - **写长文**：点输入框右下角的放大按钮进入全屏编辑（Esc 或右上角收起退出；Ctrl+Enter 直接发布）
   - **打标签**：正文里写 `#标签`（如 `#生活`），输入 `#` 会自动弹出已有标签候选，选中即可；也可以直接手打完整标签
   - **本地上传**：点"本地上传"，选图（自动压缩到 1600px / JPEG 0.8，**单条最多 4 张**）
   - **图床链接**：点"图床链接"，粘贴 `https://...` 图片 URL（只存链接，不占仓库，**单条最多 20 张**）
4. 点"发布"

其他：
- 删除：悬停 memo 出现垃圾桶按钮（需已连接）
- 引用：点引用按钮，回复别人的 memo；点引用标识会**跳到被引用 memo 的头部**（能看到它的发布时间和作者信息）
- **标签筛选**：点正文里的 `#标签`，只显示带该标签的 memo（顶部出现 `#标签 ×` 可一键清除，地址栏带 `#tag=xxx` 可分享链接）
- **长文折叠**：超过 8 行的 memo 会默认折叠，点"展开全文"查看全文
- **多图折叠**：图片超过 9 张时先显示九宫格，点右下角"查看全部 N 张"展开其余图片
- **搜索**：页面顶部搜索框，同时搜正文和标签；和标签筛选可以叠加使用
...
| Memo 发不出去 | 检查是否已"连接"、token 是否过期；图片是否超过限制（本地上传最多 4 张、图床链接最多 20 张） |
