...
- 支持引用/回复其他 Memo；点击引用标识跳转到被引用 Memo 的**首部**（头像/时间可见，长文卡片不会落在中部）
- 支持标签：正文写 `#标签` 自动渲染为可点击标签；输入 `#` 弹出已有标签候选（↑↓ 选择、Enter 确认、Esc 关闭）；点击标签聚合显示该标签的 Memo（URL 带 `#tag=xxx`，可分享、刷新保持）
- 支持页面内搜索：列表顶部搜索框同时匹配**正文和标签**，可与标签过滤叠加；无结果显示「没有匹配的 Memo」
- 支持删除（作者本人在自己的浏览器）
...
| M4 Memo | Memo 页（时间线 UI）+ 在线发布（token/照片/图床/引用/灯箱/删除/实时刷新）+ 标签（候选/聚合）+ 页面搜索 + 引用跳转首部 | Memo 体系完成 |
...
- [x] Memo：能在线发文字/本地上传照片/图床链接/引用/删除，图片灯箱查看，刷新后实时可见，无 token 时只读；标签（`#` 候选/点击聚合）、页面搜索（正文+标签）、引用跳转卡片首部正常
